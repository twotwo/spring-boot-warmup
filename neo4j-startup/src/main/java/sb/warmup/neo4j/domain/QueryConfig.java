 ...
 * QueryConfig
 * 
 * <a href="https://stackoverflow.com/questions/30079255/propertysource-in-a-jar-for-an-external-file-on-the-classpath">参考</a>
 */
@Configuration
@PropertySources({
    @PropertySource(value = "${query.path}", ignoreResourceNotFound = true, factory = YamlPropertySourceFactory.class),
    @PropertySource(value = "classpath:application-query.yml", factory = YamlPropertySourceFactory.class) })
...
public class QueryConfig implements InitializingBean {
  ...
  public Query getQuery(String keyword) {
    return Optional.ofNullable(map.get(keyword))
        .orElseThrow(() -> new RuntimeException("keyword not found"));
  }

  public List<Query> getQueries() {
    return map.values().stream()
      .sorted(Comparator.comparing(Query::getHint))
